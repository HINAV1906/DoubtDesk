...
const EXPECTED_COLS = ['FullName', 'ShortName', 'Subject', 'MoNumber', 'Pass'];
...
const AddFacultyCsv = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res  = await fetch(`${API_BASE}/admin/add-faculty-csv/`, {
        ...
        <div className="admin-form-card" style={{ maxWidth: 620 }}>
          ...
          {result && (
            <div style={{ marginTop: '0.9rem', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              <p className="admin-msg-success">
                ...
                CSV processed successfully! {result.created} faculty member(s) added, {result.skipped} skipped. ✅
              </p>
          ...
          {file && !result && (
            <button className="admin-submit-btn" style={{ marginTop: '1rem' }} onClick={handleUpload} disabled={loading}>
              ...
              {loading ? 'Uploading…' : `Upload ${preview?.rows.length || ''} Faculty`}
            </button>
          )}
